...
def read_db_news(db_path_news, date_min, date_max=None) -> pd.DataFrame:
    """
    Читает новости из базы данных начиная с указанной даты. Если date_max не указан, читает до конца.
    """
...
def save_latest_titles_to_markdown(db_path_news: Path, db_path_quote: Path,
    ...
    df_news = read_db_news(db_path_news, date_min_gmt)
